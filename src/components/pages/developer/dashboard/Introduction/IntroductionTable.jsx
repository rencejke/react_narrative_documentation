...
const IntroductionTable = ({setItemEdit, isLoading, isFetching, intro}) => {
        ...
        <table className=' font-regular'>
                ...
                {intro?.data.length === 0 && (
                    ...
                    {intro?.data.map((item, key) => (
                        <tr key={key}>
                        <td>{counter++}</td>
                        <td>{item.intro_article}</td>
                        <td>{item.intro_is_active === 1 ? "Yes" : "No"}</td>
                        <td>{item.intro_publish_date}</td>
                        
                        
                        <td className='table-action'>
                        <ul>
                            {item.intro_is_active ? (
                                <>
                ...
                {store.isActive && <ModalConfirmed position="center"  queryKey="intro" endpoint={`/v1/intro/active/${id}`} isArchiving={isArchiving}/>}

                {store.isDelete && <ModalDelete position="center"  queryKey="intro" endpoint={`/v1/intro/${id}`} />} 
